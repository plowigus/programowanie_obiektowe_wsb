    ...
    public class HotelDatabaseManager
    {
        ...
        public void AddReservation(int roomId, string firstName, string lastName, DateTime startDate, DateTime endDate)
        {
            ...
            using (var connection = new SQLiteConnection(connectionString))
            {
                ...
                // Sprawdź, czy clientId nie został dostarczony; jeśli nie, nadaj nowy
                int clientId;
                if (string.IsNullOrEmpty(firstName) || string.IsNullOrEmpty(lastName))
                {
                    // Jeśli brakuje imienia lub nazwiska, generuj losowy numer
                    Random random = new Random();
                    clientId = random.Next(1000, 9999); // Możesz dostosować zakres do swoich potrzeb
                }
                else
                {
                    // Jeśli imię i nazwisko zostały dostarczone, użyj ich hashu jako clientId
